 ...
 * @author Kai Burjack
 */
public class VoxelGameGL implements Runnable {
    ...
    private void determineOpenGLCapabilities() {
        ...
        logger.info("useMultiDrawIndirect: " + useMultiDrawIndirect);
        logger.info("useBufferStorage: " + useBufferStorage);
        logger.info("drawPointsWithGS: " + drawPointsWithGS);
        logger.info("useInverseDepth: " + useInverseDepth);
        logger.info("useNvMultisampleCoverage: " + useNvMultisampleCoverage);
        logger.info("canUseSynchronousDebugCallback: " + canUseSynchronousDebugCallback);
        logger.info("canGenerateDrawCallsViaShader: " + canGenerateDrawCallsViaShader);
        logger.info("useOcclusionCulling: " + useOcclusionCulling);
        logger.info("useTemporalCoherenceOcclusionCulling: " + useTemporalCoherenceOcclusionCulling);
        logger.info("canSourceIndirectDrawCallCountFromBuffer: " + canSourceIndirectDrawCallCountFromBuffer);
        logger.info("useRepresentativeFragmentTest: " + useRepresentativeFragmentTest);
        logger.info("uniformBufferOffsetAlignment: " + uniformBufferOffsetAlignment);
    }
    ...
    private void createChunksProgram() throws IOException {
        ...
        int vshader = createShader("voxelgame/chunk" + (drawPointsWithGS ? "-points" : "") + ".vs.glsl", GL_VERTEX_SHADER, defines);
        int gshader = 0;
        if (drawPointsWithGS) {
            gshader = createShader("voxelgame/chunk.gs.glsl", GL_GEOMETRY_SHADER, defines);
        }
        int fshader = createShader("voxelgame/chunk.fs.glsl", GL_FRAGMENT_SHADER, defines);
    ...
    private void createSelectionProgram() throws IOException {
        int program = glCreateProgram();
        int vshader = createShader("voxelgame/selection.vs.glsl", GL_VERTEX_SHADER, Collections.emptyMap());
        int fshader = createShader("voxelgame/selection.fs.glsl", GL_FRAGMENT_SHADER, Collections.emptyMap());
        glAttachShader(program, vshader);
        glAttachShader(program, fshader);
        glLinkProgram(program);
        if (DEBUG || GLDEBUG) {
    ...
    private void createBoundingBoxesProgram() throws IOException {
        ...
        int program = glCreateProgram();
        int vshader = createShader("voxelgame/boundingboxes.vs.glsl", GL_VERTEX_SHADER, defines);
        int gshader = createShader("voxelgame/boundingboxes.gs.glsl", GL_GEOMETRY_SHADER, defines);
        int fshader = createShader("voxelgame/boundingboxes.fs.glsl", GL_FRAGMENT_SHADER, defines);
        glAttachShader(program, vshader);
        glAttachShader(program, gshader);
        glAttachShader(program, fshader);
        glLinkProgram(program);
    ...
    private void createCollectDrawCallsProgram() throws IOException {
        ...
        int program = glCreateProgram();
        int vshader = createShader("voxelgame/collectdrawcalls.vs.glsl", GL_VERTEX_SHADER, defines);
        glAttachShader(program, vshader);
        glLinkProgram(program);
        glDeleteShader(vshader);
        if (DEBUG || GLDEBUG) {
    ...
    public void run() {
        ...
        // TODO: 16/09/2021 Melhorar esse try/catch
        try {
            initGLResources();
        } catch (IOException e) {
            e.printStackTrace();
